...
// checkpointoracleabi是用于生成绑定的输入ABI。
...
// checkpointoraclefuncsigs将4个字节函数签名映射到其字符串表示形式。
...
// CheckPointoraclebin是用于部署新合同的编译字节码。
...
// DeployCheckPointoracle部署了新的以太坊合同，将检查点的实例绑定到了它。
